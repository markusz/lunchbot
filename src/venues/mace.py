...
def get_lunch_for_date(date=datetime.now(), show_only_current_day=True):
    try:
        ...
        types = re.match(r".+?(MACE Restaurant+?)(.+?)(MACE Kitchen)+?(.+?)$", reduced_string, re.IGNORECASE)

        restaurant = re.findall(r'(.+?)([0-9]+,[0-9]{1,2})',  types.group(2))
        kitchen = re.findall(r'(.+?)([0-9]+,[0-9]{1,2})',  types.group(4))
